...
public abstract class DynamicObject extends GameObject {
    ...
    protected void move(Direction direction, int distance) {
        ...
        if (isAllowedSurface(tmpXPosition, tmpYPosition) && isExit(tmpXPosition, tmpYPosition)) {
    ...
    private Boolean isExit(int x, int y) {
        return (GameMaster.getInstance().getMap().getMap()[y][x] != GameMaster.getInstance().temp_exit_character);
    }
